...
actor MemoryCoordinator {
  ...
  func recoverInterruptedExtractions() async {
    do {
      ...
      debugLog.log(
        "memory-extraction", "recover failed error=\(error.localizedDescription)", level: .error)
    ...
    while true {
      ...
      do {
        ...
        guard let project = projectsByID[task.projectID] else {
          ...
          debugLog.log(
            "memory-extraction", "drop task=\(task.id.uuidString) reason=missing-project")
          publishStatus(.idle)
          continue
        }

        let extractionProvider = providerSelectionService.preferredMemoryExtractionProvider(
        ...
        let existingUserMemories =
          (try? store.listEntries(
            scope: .user, tiers: [.working], limit: settings.memory.maxInjectedUserWorkingMemories))
          ?? []
        ...
        debugLog.log(
          "memory-extraction",
          "failed task=\(task.id.uuidString) error=\(error.localizedDescription)", level: .error)
        publishStatus(.failed)
      }
    }
    publishStatus(.idle)
  ...
  private func apply(
    ...
    try store.trimWorkingEntries(
      scope: .project, projectID: task.projectID, maxActive: settings.memory.maxActiveWorkingEntries
    )
  }
  ...
  private func resolveTranscriptSource(
    ...
    let sessionID = normalizedNonEmptyString(session.aiSessionID) ?? session.terminalID.uuidString
    ...
    switch tool {
    case "claude":
      guard let aiSessionID = normalizedNonEmptyString(session.aiSessionID) else { return nil }
      let path = AIRuntimeSourceLocator.claudeSessionLogURL(
    ...
    case "codex":
      ...
      return transcriptSourceIfReadable(path: fileURL.path, tool: tool, sessionID: aiSessionID)
  ...
  private func fetchOpenCodeTranscript(
    ...
    while sqlite3_step(statement) == SQLITE_ROW {
      ...
      if !content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        let prefix = partType == "tool" && toolName.isEmpty == false ? "\(role).tool[\(toolName)]" : role
