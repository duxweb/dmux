...
如果只是快速试用，可以直接在 **设置 > 远程** 中填写官网试用节点 `https://codux-node.dux.plus`。生产环境或长期使用建议部署自己的 `codux-service`。

### 远程配置流程

| 步骤 | 操作 |
|:--|:--|
| 1 | 使用官网试用节点 `https://codux-node.dux.plus`，或在自己的服务器、VPS、局域网机器上部署 `codux-service` |
